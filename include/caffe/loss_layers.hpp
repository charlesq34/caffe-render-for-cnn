...
namespace caffe {
  ...
  /**
   * Author: Charles R. Qi
   * @brief: extension of softmax loss layer, used for viewpoint estimation
  ...
  /**
   * Author: Charles R. Qi
   * @brief: softmax loss layer with one extension: support pos and neg loss weight.
   *
   */
  template <typename Dtype>
    class SoftmaxPosNegWithLossLayer : public LossLayer<Dtype> {
      public:
        explicit SoftmaxPosNegWithLossLayer(const LayerParameter& param)
          : LossLayer<Dtype>(param),
          softmax_layer_(new SoftmaxLayer<Dtype>(param)) {}
        virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
            const vector<Blob<Dtype>*>& top);
        ...
        virtual inline LayerParameter_LayerType type() const {
          return LayerParameter_LayerType_SOFTMAX_POS_NEG_LOSS;
        }
        virtual inline int ExactNumBottomBlobs() const { return -1; }
        virtual inline int MinBottomBlobs() const { return 2; }
        virtual inline int MaxBottomBlobs() const { return 3; }
      ...
      protected:
        /// @copydoc SoftmaxPosNegWithLossLayer
        virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
            const vector<Blob<Dtype>*>& top);
  ...
  /**
   * Author: Charles R. Qi
   * @brief Computes the average viewpoint classification accuracy. (category known by label)
   */
  template <typename Dtype>
    class AccuracyViewLayer : public Layer<Dtype> {
      public:
        ...
        virtual inline LayerParameter_LayerType type() const {
          return LayerParameter_LayerType_ACCURACY_VIEW;
        }
      ...
      protected:
        ...
        /// @brief Not implemented -- AccuracyViewLayer cannot be used as a loss.
        virtual void Backward_cpu(const vector<Blob<Dtype>*>& top,
            const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
          for (int i = 0; i < propagate_down.size(); ++i) {
            if (propagate_down[i]) { NOT_IMPLEMENTED; }
  ...
  /**
   * author: charles r. qi
   * split label to det and view
   */
  template <typename Dtype>
    class DetViewSplitLayer : public Layer<Dtype> {
      public:
        explicit DetViewSplitLayer(const LayerParameter& param)
          : Layer<Dtype>(param) {}
        virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
            const vector<Blob<Dtype>*>& top);
        virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
        ...
        virtual inline LayerParameter_LayerType type() const {
          return LayerParameter_LayerType_DET_VIEW_SPLIT;
        }
